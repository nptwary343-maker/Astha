...
export default function FloatingActionButtons() {
    ...
    return (
        <div className="fixed bottom-24 md:bottom-10 right-6 z-[100] flex flex-col gap-4 items-end">
            ...
            >
                {/* Initial "Need Help?" Tooltip bubble */}
                <div className={`absolute right-[120%] bg-white border border-gray-100 shadow-2xl rounded-2xl p-3 w-48 transition-all duration-700 pointer-events-none origin-right
                ...
                `}>
                    ...
                    {/* Arrow tail */}
                    <div className="absolute right-[-6px] top-1/2 -translate-y-1/2 w-4 h-4 bg-white border-r border-t border-gray-100 rotate-45 z-[-1]" />
                </div>

                {/* Main Phone Button */}
                <a
                    href={`tel:${phone}`}
                    className="bg-gray-900 text-white p-4 rounded-full shadow-[0_10px_30px_-10px_rgba(31,41,55,0.8)] hover:scale-105 transition-all flex items-center justify-center group h-14 w-14 md:h-16 md:w-16 z-20 overflow-hidden"
                >
